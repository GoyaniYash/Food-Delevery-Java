    ...
    static void addRestaurant() {
        ...
        try {
            ...
            if (generatedKeys.next()) {
                ...
                System.out.println(
                        "Enter Menu Items name and price. IF No Other Items Add When Type 'done' when finished.");
                while (true) {
                ...
                if (restaurantFeedbackChoice.equalsIgnoreCase("yes")) {
                    for (int restaurantId : restaurantIds) {
                        ...
                        try (PreparedStatement feedbackStatement = connection.prepareStatement(feedbackQuery)) {
                            ...
                            try (PreparedStatement restaurantStatement = connection
                                    .prepareStatement(updateRestaurantQuery)) {
    ...
    private static void showMostRatedRestaurant() {
        ...
        try (Connection connection = JDBCUtils.getConnection();
            ...
            for (RestaurantRating rating : ratings) {
                System.out
                        .println("Restaurant ID: " + rating.restaurantId + " with Average Rating: " + rating.avgRating);
